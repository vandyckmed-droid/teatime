...
(async () => {
  ...
  // (The held-vs-plan card that once followed the plan here was removed with
  // the holdings record — the owner chose not to maintain per-position data.
  // Its element is gone from the page; the check below would catch a zombie.)
  check('no held-vs-plan card remains', await page.evaluate(() => !document.getElementById('watchlist-held')));
